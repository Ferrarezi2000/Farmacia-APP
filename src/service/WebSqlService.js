export default class ClienteService {
    constructor() {
        if (window.openDatabase) {
            this._mydb = openDatabase("app_db", "1.0", "Banco de dados do APP", 4 * 1024 * 1024);
        } else {
            alert("WebSQL não é suportado! Vai dar merda. É melhor fechar isso aí!");
        }
    }

    execSql(sql, param, callback) {
        if (this._mydb) {
            this._mydb.transaction(function (t) {
                t.executeSql(sql, param, callback);
            });
        } else {
